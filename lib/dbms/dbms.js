...
function Dbms() {
     ...
     * @params string options.webapp - Optional port to serve the admin web UI, otherwise will not serve
    ...
    self.initialize = function (options, callback) {
        ...
        steps.push(function startDbms(cb) {
            self._net = net.createServer(function (con) {
                var dbms = rpc(rpcRoutes(password, con, self._env, self.model));
        ...
        if (options.webapp) {
            steps.push(function startWebAppServer(cb) {
                webapp(self).listen(options.webapp, function (err) {
                    if (err) {
                        debug('failed webap listen', err);
                    } else {
                        debug('webapp is listening on port', options.webapp);
                    }
                });
            });
        }
