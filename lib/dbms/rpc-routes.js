...
module.exports = function routeGenerator(password, con, env, model) {
    ...
    return {
        ...
        getByIndex: route(function getByIndexRoute(body, next) {
            ...
            var txn = env.beginTxn();
            var indexStore = model(body.model).indexDbi;
            var cursor = new lmdb.Cursor(txn, indexStore);
            var realIds = [];
            var _id;
            var index;

            async.each(body.indexes, function eachIndex(index, cb) {
                cursor.goToFirst();
                var noMoreFound = false;
                async.whilst(
                    function () { return noMoreFound; },
                    function (callb) {
                        cursor.goToKey(index);
                        cursor.getCurrentString(function (key, val) {
                            if (!key || !val) { noMoreFound = true; }
                            else {
                                realIds.push(val);
                            }
                            callb();
                ...
                // Now that we have all of the ids fetched, we can
                // fetch these records
