...
module.exports = function routeGenerator(password, con, env, model) {
    ...
    return {
        ...
        /**
         * Put data. { model: '', id: '932ksblkas', data: { any: "json" }, indexes: [], uniques: [] }
         */
        put: route(function putRoute(body, next) {
            ...
            if (body.uniques && !(body.uniques instanceof Array)) {
                var err = new Error('uniques should be an array');
                err.status = 400;
                return next(err);
            }
            ...
            for (var i = 0; i < uniques.length; i++) {
                ...
                if (exists && exists !== id) {
                    uniqueFailed = new Error('Value already exists: ' + val.split(':')[0]);
        ...
        /**
         * Get data by the index. `{ model: '', indexes: ['someprop:Billy Bob'] }`
         * Responds with `{ custom: 'stored', properties: 'here' }`
         */
        getByIndex: route(function getByIndexRoute(body, next) {
            debug('getByIndex', body);
            if (!body.model) {
                var err = new Error('Missing model');
                err.status = 400;
                return next(err);
            ...
            for (var i = 0; i < body.indexes.length; i++) {
                _id = txn.getString(indexStore, body.index);
